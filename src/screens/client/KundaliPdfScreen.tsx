...
export function KundaliPdfScreen() {
  ...
  const submit = async () => {
    ...
    const payload: KundaliPdfPayload = {
      birth: {
    ...
    try {
      setSubmitting(true);
      const response = await generateKundaliPdf(payload);
      setResult(response, payload);
      router.push("/kundali-pdf-result");
    } catch (error) {
      setSubmitError(getApiErrorMessage(error, "Unable to generate Kundali PDF"));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <SafeAreaView style={styles.root} edges={["top", "left", "right"]}>
      ...
      >
          <View style={styles.intro}>
            <Text variant="headlineSmall" style={styles.introTitle} numberOfLines={3} adjustsFontSizeToFit minimumFontScale={0.72}>{t("Create Kundali PDF")}</Text>
